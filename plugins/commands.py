        ...
        try: 
         ...
         #       newPoint = await db.get_point(refUserId)
             if AUTH_CHANNEL and await is_req_subscribed(client, message):
                        ...
                        reply_markup = InlineKeyboardMarkup(buttons)
                        m=await message.reply_sticker("CAACAgUAAxkBAAID8mdSxD4GPdjmldB3VywhacWXwyAZAAIKAAPBJDExDF3pk_t_hK4eBA") 
...
async def set_shortner(c, m):
    ...
    try:
        URL = m.command[1]
        API = m.command[2]
        resp = requests.get(f'https://{URL}/api?api={API}&url=https://telegram.dog/CINEMACCBOTDISCUSSION').json()
...
async def set_shortner_2(c, m):
    ...
    try:
        URL = m.command[1]
        API = m.command[2]
        resp = requests.get(f'https://{URL}/api?api={API}&url=https://telegram.dog/CINEMACCBOTUPDATES').json()
        if resp['status'] == 'success':
            SHORT_LINK = resp['shortenedUrl']
